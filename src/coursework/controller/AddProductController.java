...
public class AddProductController implements Initializable {
    ...
    private boolean validateAuthor() {
        ...
        if (m.find() && m.group().equals(bookAuthor.getText())) {
            ...
            coursework.model.Alert alert = new coursework.model.Alert(Alert.AlertType.ERROR, "Book author validation", "Please enter a valid book author!");
            return false;
        }
    }
    ...
    private boolean validatePublisher() {
        ...
        if (m.find() && m.group().equals(bookPublusher.getText())) {
            ...
            coursework.model.Alert alert = new coursework.model.Alert(Alert.AlertType.ERROR, "Book publisher validation", "Please enter a valid book publisher!");
            return false;
        }
    }
    ...
    private boolean validateQuantity() {
        ...
        if (m.find() && m.group().equals(quantity.getText())) {
            ...
            coursework.model.Alert alert = new coursework.model.Alert(Alert.AlertType.ERROR, "Book quantity validation", "Please enter a valid book quantity!");
            return false;
        }
    }
    ...
    private boolean validateBookQuantity() {
        ...
        if (bookQuantity == 0) {
            coursework.model.Alert alert = new coursework.model.Alert(Alert.AlertType.ERROR, "Book quantity validation", "Book quantity cannot be zero!");
            return false;
        } else {
            return true;
        }
        ...
        try {
            ...
            if (validateName() && validateAuthor() && validatePublisher() && validateEdition() && validateQuantity() && validateBookQuantity()) {
                ...
                if (BookQuantity < issuedBooks) {
                    coursework.model.Alert alert = new coursework.model.Alert(Alert.AlertType.ERROR, "Error", "Book quantity cannot be less than issued books");
                } else {
                    pre = conn.prepareStatement(query);
